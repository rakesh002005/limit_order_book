...
public:
    void purge() {
        for (auto k = buy.begin(); k != buy.end(); ++k) {
            for (auto it = k->second.begin(); it != k->second.end();) {
                if (it->quantity == 0) {
                    cancel_map.erase(it->id);
                    it = k->second.erase(it);
                } else it++;
            }
        }

        for (auto k = sell.begin(); k != sell.end(); ++k) {
            for (auto it = k->second.begin(); it != k->second.end();) {
                if (it->quantity == 0) {
                    cancel_map.erase(it->id);
                    it = k->second.erase(it);
   ...
   void simulate_order_book(int num_orders) {
    ...
    for (int i = 0; i < num_orders; ++i) {
        ...
        if (action < 40) {
            // BUY LIMIT ORDER
            int64_t price = 10000 + rand() % 1000;
            double quantity = static_cast<double>(1 + rand() % 50); // CHANGED
            order buy_order = {order_id++, price, quantity,
            ...
            int64_t price = 10000 + rand() % 1000;
            double quantity = static_cast<double>(1 + rand() % 50); // CHANGED
            order sell_order = {order_id++, price, quantity,
            ...
            if (!sell.empty()) {
                double quantity = static_cast<double>(1 + rand() % 50); // CHANGED
                order buy_order = {order_id++, 11000, quantity,
            ...
            if (!buy.empty()) {
                double quantity = static_cast<double>(1 + rand() % 50); // CHANGED
                order sell_order = {order_id++, 0, quantity,
